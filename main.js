...
const infoUser = (id) => {
  ...
  user.status
    ? statusInfo.classList.add("activeStatusInfo")
    : statusInfo.classList.add("inactiveStatusInfo");
};
...
const searchUser = (searchValue) => {
  const data2 = data.filter((elem) =>
    elem.name.toLowerCase().trim().includes(searchValue.toLowerCase().trim())
  );
  getData(data2);
};
inputSearch.oninput = () => {
...
/// status filter
const filterUserStatus = (statusValue) => {
  const data2 = data.filter((elem) => {
...
const getData = (data) => {
  ...
  data.forEach((elem) => {
    ...
    elem.status == true
      ? status.classList.add("activeStatusInfo")
      : status.classList.add("inactiveStatusInfo");
